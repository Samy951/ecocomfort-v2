...
class WebSocketService {
  ...
  private lastUserInit: { userId: string; organizationId: string } | null = null;
  ...
  private connect() {
    try {
      ...
      const httpOrigin = import.meta.env.VITE_API_ORIGIN || 'http://localhost:3000';
      const socketUrl = envUrl && envUrl.length > 0 ? envUrl : httpOriginToWs(httpOrigin);
      ...
      this.socket.on('connect', () => {
        ...
        if (this.pendingUserInit) {
          ...
          if (import.meta.env.DEV) {
            console.log("👤 Ré-émission de l'initialisation utilisateur persistée:", this.lastUserInit);
          }
      ...
      this.socket.on('sensor-data-updated', (data) => {
        if (import.meta.env.DEV) console.log('📊 Données capteur mises à jour:', data);
  ...
  public emit(eventType: string, data: any) {
    if (this.socket && this.isConnected) {
      ...
      if (import.meta.env.DEV) console.warn("⚠️ Socket.IO non connecté, impossible d'émettre:", eventType);
    }
  }

  public initializeUser(userId: string, organizationId: string) {
    if (import.meta.env.DEV) {
      console.log(`👤 Initialisation utilisateur: ${userId}, organisation: ${organizationId}`);
    }
    if (this.isConnected && this.socket) {
      this.emit('user_init', { userId, organizationId });
      this.lastUserInit = { userId, organizationId };
    } else {
      if (import.meta.env.DEV) console.log('⏳ Connexion Socket.IO en cours, initialisation différée...');
