...
const Dashboard = ({ setIsConnected, gamification, currentUser }: DashboardProps) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeDoorState = webSocketService.on('door-state-certainty-changed', (event: any) => {
      console.log('🚪 Door state certainty changed:', event)
      // Force immediate refresh to bypass timing issues
      refreshSensorsImmediate()
    })
